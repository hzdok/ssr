...
Download_SSR(){
	cd "/usr/local/"
                wget -N --no-check-certificate "https://github.com/shadowsocksr-backup/shadowsocksr/archive/3.1.2.zip"
	...
	[[ ! -e "3.1.2.zip" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 下载失败 !" && rm -rf 3.1.2.zip && exit 1
	unzip "3.1.2.zip"
	[[ ! -e "/usr/local/shadowsocksr-3.1.2/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf 3.1.2.zip && exit 1
	mv "/usr/local/shadowsocksr-3.1.2/" "/usr/local/shadowsocksr/"
